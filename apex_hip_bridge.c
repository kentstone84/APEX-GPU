...
static int load_hip_library(void)
{
    ...
    // Try to load HIP runtime library
    hip_handle = dlopen("libamdhip64.so", RTLD_LAZY);
    if (!hip_handle) {
        hip_handle = dlopen("libamdhip64.so.6", RTLD_LAZY);
    }
    if (!hip_handle) {
    ...
    if (!hip_handle) {
        fprintf(stderr, "[HIP-BRIDGE] ❌ Failed to load HIP library: %s\n", dlerror());
        return 0;
    }

    // Load function pointers
    real_hipMalloc = (hipMalloc_t)dlsym(hip_handle, "hipMalloc");
    real_hipFree = (hipFree_t)dlsym(hip_handle, "hipFree");
    real_hipMemcpy = (hipMemcpy_t)dlsym(hip_handle, "hipMemcpy");
    real_hipDeviceSynchronize = (hipDeviceSynchronize_t)dlsym(hip_handle, "hipDeviceSynchronize");
    real_hipGetDeviceCount = (hipGetDeviceCount_t)dlsym(hip_handle, "hipGetDeviceCount");
    real_hipGetDeviceProperties = (hipGetDeviceProperties_t)dlsym(hip_handle, "hipGetDeviceProperties");
    real_hipSetDevice = (hipSetDevice_t)dlsym(hip_handle, "hipSetDevice");
    real_hipGetLastError = (hipGetLastError_t)dlsym(hip_handle, "hipGetLastError");
    real_hipGetErrorString = (hipGetErrorString_t)dlsym(hip_handle, "hipGetErrorString");
    real_hipMemset = (hipMemset_t)dlsym(hip_handle, "hipMemset");
    real_hipStreamCreate = (hipStreamCreate_t)dlsym(hip_handle, "hipStreamCreate");
    real_hipStreamDestroy = (hipStreamDestroy_t)dlsym(hip_handle, "hipStreamDestroy");
    real_hipStreamSynchronize = (hipStreamSynchronize_t)dlsym(hip_handle, "hipStreamSynchronize");
    real_hipLaunchKernel = (hipLaunchKernel_t)dlsym(hip_handle, "hipLaunchKernel");

    // Load new functions
    real_hipEventCreate = (hipEventCreate_t)dlsym(hip_handle, "hipEventCreate");
    real_hipEventDestroy = (hipEventDestroy_t)dlsym(hip_handle, "hipEventDestroy");
    real_hipEventRecord = (hipEventRecord_t)dlsym(hip_handle, "hipEventRecord");
    real_hipEventSynchronize = (hipEventSynchronize_t)dlsym(hip_handle, "hipEventSynchronize");
    real_hipEventElapsedTime = (hipEventElapsedTime_t)dlsym(hip_handle, "hipEventElapsedTime");
    real_hipEventQuery = (hipEventQuery_t)dlsym(hip_handle, "hipEventQuery");
    real_hipMemcpyAsync = (hipMemcpyAsync_t)dlsym(hip_handle, "hipMemcpyAsync");
    real_hipMemsetAsync = (hipMemsetAsync_t)dlsym(hip_handle, "hipMemsetAsync");
    real_hipMallocPitch = (hipMallocPitch_t)dlsym(hip_handle, "hipMallocPitch");
    real_hipMemcpy2D = (hipMemcpy2D_t)dlsym(hip_handle, "hipMemcpy2D");
    real_hipHostMalloc = (hipHostMalloc_t)dlsym(hip_handle, "hipHostMalloc");
    real_hipHostFree = (hipHostFree_t)dlsym(hip_handle, "hipHostFree");
    real_hipGetDevice = (hipGetDevice_t)dlsym(hip_handle, "hipGetDevice");
    real_hipDeviceReset = (hipDeviceReset_t)dlsym(hip_handle, "hipDeviceReset");
    real_hipDeviceGetAttribute = (hipDeviceGetAttribute_t)dlsym(hip_handle, "hipDeviceGetAttribute");
    real_hipMemGetInfo = (hipMemGetInfo_t)dlsym(hip_handle, "hipMemGetInfo");

    if (!real_hipMalloc || !real_hipFree || !real_hipMemcpy) {
        fprintf(stderr, "[HIP-BRIDGE] ❌ Failed to load required HIP symbols\n");
        dlclose(hip_handle);
        hip_handle = NULL;
        return 0;
    }
...
void apex_hip_init(void)
{
    ...
    if (real_hipGetDeviceCount && real_hipGetDeviceCount(&device_count) == 0) {
        ...
        if (device_count > 0 && real_hipGetDeviceProperties) {
            ...
            struct {
                char name[1024];  // FIXED: Increased from 256 to 1024
